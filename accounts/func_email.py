...
def generate_sales_email(data):
    ...
    for i, Prospect_Title in enumerate(data["Prospect_Title_list"]):
        ...
        # Store the generated email in the dictionary format
        email_dict = {
            "To": Prospect_Title,
            "Subject": response.choices[0].text.split("\n", 1)[0],  # Extract the first line as the subject
            "Body": response.choices[0].text
        }
        generated_emails_list.append(email_dict)
        # print(generated_emails_dict)
        # return generated_emails_dict
        return generated_emails_list
...
# generated_emails_dict = generate_sales_email(data)
# print(generated_emails_dict)